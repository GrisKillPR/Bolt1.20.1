...
execute unless score pointsToWin gm_options matches 20 at @e[type=area_effect_cloud,tag=marker,tag=option,tag=option_points_to_win] run setblock ~ ~ ~ minecraft:oak_wall_sign[facing=west]{front_text: {color: "aqua", messages: ['{"text":"Points to"}', '{"text":"win"}', '{"score":{"name":"pointsToWin","objective":"gm_options"},"color":"gold"}', '{"text":"","clickEvent":{"action":"run_command","value":"trigger lb_optionTrig set 2"}}']}} destroy
