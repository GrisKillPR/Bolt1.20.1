...
#execute at @e[type=area_effect_cloud,tag=marker,tag=option,tag=option_points_to_win] run setblock ~ ~ ~ minecraft:oak_wall_sign[facing=west]{front_text: {messages: ['{"color":"aqua","text":"Flag Captures"}', '{"color":"aqua","text":"text":"to win"}', '{"score":{"name":"pointsToWin","objective":"gm_options"},"color":"gold"}', '{"text":""}']}} destroy
