...
execute if score tabScoreboard gm_options matches 0 at @e[type=area_effect_cloud,tag=marker,tag=option,tag=option_tab_board] run setblock ~ ~ ~ minecraft:oak_wall_sign[facing=west]{front_text: {color: "aqua", messages: ['{"text":"Score in tab"}', '{"text":""}', '{"text":"Kills","color":"green"}', '{"text":"Default","color":"green","clickEvent":{"action":"run_command","value":"trigger lb_optionTrig set 6"}}']}} destroy
