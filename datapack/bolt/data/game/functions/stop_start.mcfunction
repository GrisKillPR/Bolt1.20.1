...
execute as @e[type=minecraft:text_display,tag=start,tag=sign] run data merge entity @s {CustomName:'{"text":"Start Game","color":"gold","bold":true}'}
...
#unpower button
execute at @e[type=minecraft:text_display,tag=start,tag=sign] run setblock ~ ~ ~ minecraft:stone_button[face=floor,facing=east,powered=false]